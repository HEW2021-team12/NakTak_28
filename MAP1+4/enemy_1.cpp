...
HRESULT InitEnemy(void)
{
	...
	for (int i = 0; i < ENEMY_MAX; i++)
	{
		...
		{
			g_Enemy[i].use = true;
			g_Enemy[i].routenom = ROOT_NUM;

			g_Enemy[i].pos.x = g_RouteX[ROOT_NUM] * CHIP_SIZE + (CHIP_SIZE / 2);
			g_Enemy[i].pos.y = g_RouteY[ROOT_NUM] * CHIP_SIZE + (CHIP_SIZE / 2);
		}
		// 二体目
		if (i == 1)
		{
			g_Enemy[i].use = false;
			g_Enemy[i].routenom = ROOT_NUM2;
...
void WatchEnemy(int i)
{
	// 旧版サーチ
	{
		...
		if (g_Enemy[i].movecntX)
		{
			...
			g_Enemy[i].routenom = work;
			g_Enemy[i].pos.x = g_RouteX[work] * CHIP_SIZE + (CHIP_SIZE / 2);
			g_Enemy[i].pos.y = g_RouteY[work] * CHIP_SIZE + (CHIP_SIZE / 2);
		}
		if (g_Enemy[i].movecntY)
		{
			g_Enemy[i].movecntY = false;

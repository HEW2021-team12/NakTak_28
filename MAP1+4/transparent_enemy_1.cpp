//=============================================================================
//
// エネミー(透明)処理 [transparent_enemy.cpp]
// Author : 古市
//
//=============================================================================
...
// メモリ確保用アドレス
ENEMY* g_Transparent;			  // エネミー構造体
char* g_Transparent_RouteX;		  // 巡回ルート用
char* g_Transparent_RouteY;		  // 巡回ルート用
...
void DrawTransparent(void)
{
	for (int i = 0; i < TRANSPARENT_MAX; i++)
	{
		if (g_Transparent[i].use == true)
		{
			...
			{
				//エネミーの位置やテクスチャー座標を反映
				float px = SCREEN_WIDTH / 2 + player->difference.x + (g_Transparent[i].pos.x - player->pos.x);	// エネミーの表示位置X
				float py = SCREEN_HEIGHT / 2 + player->difference.y + (g_Transparent[i].pos.y - player->pos.y);	// エネミーの表示位置Y
				float pw = g_Transparent[i].w;		// エネミーの表示幅
				float ph = g_Transparent[i].h;		// エネミーの表示高さ
				D3DXCOLOR col;
...
{
	// もし未使用のデータを探す
	if (g_Transparent[i].use == false)	// 未使用状態の敵データを見つける
	{
		g_Transparent[i].use = true;		// 使用状態へ変更する
